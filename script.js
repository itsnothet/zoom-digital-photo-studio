(function () {
  ...
  // data/catalogue-images.json below — this array only holds layout info,
  ...
  /* ---------- Load real photos (added via the /admin page) ---------- */
  // data/catalogue-images.json is written by the Decap CMS admin page at
  // /admin — each key is a category slug, value is a list of image paths.
  // If the file is missing or empty (nothing added yet), the catalogue and
  // galleries simply fall back to their placeholder slots.
  renderCatalogue();
  fetch("data/catalogue-images.json", { cache: "no-store" })
    .then(function (res) { return res.ok ? res.json() : {}; })
    .then(function (imagesBySlug) {
      catalogueData.forEach(function (cat) {
        var list = imagesBySlug[cat.slug];
        if (Array.isArray(list) && list.length) cat.images = list;
      });
      renderCatalogue();
    })
    .catch(function () { /* no data file yet — placeholders already rendered */ });

  /* ---------- Load site photos: logo, hero, about, services ---------- */
  // data/site-images.json is written by the "Site Photos" section of the
  // /admin page — one key per photo slot. Any slot left empty keeps its
  // placeholder. Click an existing photo in /admin to replace it.
  ...
  function setLogo(markEl, textEl, imgEl, src) {
    if (!src || !imgEl) return;
    imgEl.src = src;
  ...
  fetch("data/site-images.json", { cache: "no-store" })
    ...
    .then(function (photos) {
      photos = photos || {};
      setLogo(
        document.getElementById("brandMarkHeader"),
        document.getElementById("brandTextHeader"),
        document.getElementById("brandLogoHeader"),
        photos.logo
      );
      setLogo(
        document.getElementById("brandMarkFooter"),
        document.getElementById("brandTextFooter"),
        document.getElementById("brandLogoFooter"),
        photos.logo
      );
      setPhoto(document.getElementById("heroPhotoMain"), photos.heroMain);
      setPhoto(document.getElementById("heroPhotoSide"), photos.heroSide);
      setPhoto(document.getElementById("aboutPhoto"), photos.about);
      setPhoto(document.getElementById("svcPassport"), photos.servicePassport);
      setPhoto(document.getElementById("svcVisa"), photos.serviceVisa);
      setPhoto(document.getElementById("svcWedding"), photos.serviceWedding);
      setPhoto(document.getElementById("svcFrames"), photos.serviceFrames);
      setPhoto(document.getElementById("svcVideography"), photos.serviceVideography);
      setPhoto(document.getElementById("svcAlbums"), photos.serviceAlbums);
      setPhoto(document.getElementById("svcPortraits"), photos.servicePortraits);
      setPhoto(document.getElementById("svcProduct"), photos.serviceProduct);
    })
    .catch(function () { /* no data file yet — placeholders already rendered */ });
